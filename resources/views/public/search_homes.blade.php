...
<div class="px-5 lg:px-10 py-8 max-w-[1400px] mx-auto">
    ...
    {{-- HERO: full-bleed photo, gradient overlay, info anchored at bottom --}}
    @if($featured)
    @php
    ...
    <section class="relative rounded-3xl overflow-hidden min-h-[580px] flex flex-col justify-end shadow-2xl mb-10">
        ...
        <div class="absolute top-6 left-6 z-10">
            <span class="text-[10px] font-bold tracking-[0.22em] uppercase text-white/70 bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-4 py-1.5">
                Slideshow Gallery
            </span>
        </div>
        ...
        <div class="absolute top-6 right-6 z-10">
            <form method="GET" action="">
                <div class="flex rounded-full overflow-hidden shadow-lg bg-white/15 backdrop-blur-md border border-white/25">
                    <input
                        type="text"
                        name="q"
                        value="{{ $searchValue }}"
                        placeholder="Search address, city, zip, agent…"
                        class="bg-transparent text-white placeholder-white/60 text-sm px-5 py-3 outline-none w-72"
                    >
                    <button type="submit"
                            class="bg-white text-blue-900 font-semibold text-sm px-5 m-1 rounded-full hover:bg-blue-50 transition-colors">
                        Search
                    </button>
                </div>
            </form>
        ...
        {{-- Bottom info row --}}
        <div class="relative z-10 p-8 flex flex-col sm:flex-row sm:items-end justify-between gap-6">

            {{-- Address + price + CTA --}}
            <div>
                <div class="text-xs font-bold tracking-widest uppercase text-amber-400 mb-2">
                    Featured Listing
                </div>
                <h1 class="text-4xl lg:text-5xl font-bold text-white leading-tight drop-shadow-md">
                    {{ $fStreet }}
                </h1>
                <div class="text-white/70 text-lg mt-1">{{ $fCity }}</div>
                @if($fPrice)
                    <div class="mt-2 text-3xl font-bold text-white">{{ $fPrice }}</div>
                @endif
                <a href="{{ $fURL }}" target="_blank"
                   class="inline-flex items-center gap-2 mt-5 bg-amber-400 hover:bg-amber-300 text-slate-900 font-bold text-sm px-6 py-3 rounded-full transition-colors shadow-lg">
                    View Listing
            ...
            {{-- Agent block --}}
            <div class="flex items-center gap-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-5 py-4 shrink-0">
                @if($fAgt)
                    <img src="{{ $fAgt }}" alt="{{ $featured->theAgent->agtFullName }}"
                         class="w-20 h-20 rounded-xl object-cover border-2 border-white/30 shadow-lg">
                @else
                    <div class="w-20 h-20 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                        </svg>
                    </div>
                @endif
                <div>
                    <div class="text-[10px] font-bold tracking-widest uppercase text-white/50 mb-1">Listed by</div>
                    <div class="text-white font-semibold text-base leading-snug">
                        {{ $featured->theAgent->agtFullName }}
                    </div>
                    <div class="text-white/60 text-sm mt-0.5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
        ...
        <div>
            ...
            <a href="{{ $url }}" target="_blank"
               class="flex bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200/80 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200 group">

                {{-- Thumbnail --}}
                @if($img)
                    <img src="{{ $img }}" alt="{{ $street }}"
                         class="w-52 min-w-[13rem] h-44 object-cover group-hover:scale-[1.02] transition-transform duration-300">
                @else
                    <div class="w-52 min-w-[13rem] h-44 bg-slate-100 flex items-center justify-center shrink-0">
                ...
                {{-- Info --}}
                <div class="flex flex-1 items-center px-6 py-4 gap-6">

                    {{-- Address + price --}}
                    <div class="flex-1 min-w-0">
                        <div class="text-[17px] font-bold text-slate-800 leading-snug truncate">{{ $street }}</div>
                        <div class="text-sm text-slate-500 mt-0.5">{{ $city }}</div>
                        @if($price)
                            <div class="mt-2 text-xl font-bold text-blue-700">{{ $price }}</div>
                        @endif
                    </div>

                    {{-- Agent --}}
                    <div class="flex items-center gap-3 shrink-0 border-l border-slate-100 pl-6">
                        @if($agt)
        ...
        <aside class="space-y-5">
            ...
            <div class="bg-gradient-to-br from-blue-900 to-blue-700 rounded-2xl p-6 text-white shadow-lg">
                ...
                >
                <button class="w-full bg-amber-400 hover:bg-amber-300 text-slate-900 font-bold text-sm py-3 rounded-xl transition-colors">
                    Start Free →
                </button>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80">
                ...
                <form method="GET" action="">
                    ...
                    >
                    <button type="submit"
                            class="w-full bg-blue-700 hover:bg-blue-800 text-white font-semibold text-sm py-3 rounded-xl transition-colors">
                        Search Listings
                    </button>
                </form>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-blue-800">{{ $stat['num'] }}</div>
