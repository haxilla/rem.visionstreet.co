                            ...
                            @foreach($flyers as $flyer)
                                ...
                                <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                                    <div class="grid grid-cols-1 lg:grid-cols-[260px_1fr]">
                                        ...
                                        <div class="relative min-h-[220px] bg-slate-200">
                                            @if($img)
                                                <img
                                                    src="{{ $img }}"
                                                ...
                                                >
                                            @else
                                                <div class="flex h-full min-h-[220px] items-center justify-center bg-slate-100 text-sm font-semibold text-slate-400">
                                                    No Photo
                                                </div>
                                            @endif
                                        ...
                                        <div class="p-6">
                                                ...
                                                <div class="flex gap-2">
                                                    @if($flyer->url_slug)
                                                        <a
                                                            ...
                                                            class="rounded-xl bg-[#214e9b] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#183b78]"
                                                        >
                                                            View Flyer
                                                        </a>
                                                    @endif
                                            ...
                                            <div class="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                                                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Campaigns</div>
                                                    <div class="mt-1 text-2xl font-black text-slate-900">{{ $flyerCampaigns->count() }}</div>
                                                </div>

                                                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Delivered</div>
                                                    <div class="mt-1 text-2xl font-black text-slate-900">{{ number_format($flyerCompleted->sum('totalEmails')) }}</div>
                                                </div>

                                                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Views</div>
                                                    <div class="mt-1 text-2xl font-black text-slate-900">{{ number_format(optional($stats)->xWebViews ?? 0) }}</div>
                                                </div>

                                                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Last Sent</div>
                                                    <div class="mt-1 text-sm font-bold text-slate-900">
                                                        {{ optional($stats)->xLastDeliveryDate ? $dateFmt($stats->xLastDeliveryDate) : 'Not yet' }}
                                                    </div>
                                                </div>
                                            </div>

                                            @if($flyerCampaigns->count())
                                                <div id="campaigns" class="mt-6 overflow-hidden rounded-2xl border border-slate-200">
                                                    ...
                                                    <div class="divide-y divide-slate-200">
                                                        @foreach($flyerCampaigns as $camp)
                                                            @php
                                                                if (empty($camp->emStart) && empty($camp->emComplete)) {
                                                                    $statusText = 'Requested';
                                                                    $statusClass = 'bg-amber-100 text-amber-800';
                                                                } elseif (!empty($camp->emStart) && empty($camp->emComplete)) {
                                                                    $statusText = 'Sending';
                                                            ...
                                                            <div class="grid grid-cols-1 gap-4 px-4 py-4 lg:grid-cols-[1fr_160px_160px_120px] lg:items-center">
                                                                <div>
                                                                    ...
                                                                    <div class="mt-2 text-sm font-bold text-slate-900">
                                                                        {{ $camp->emSubject ?: 'No subject saved' }}
                                                                    </div>

                                                                    <div class="mt-1 text-xs text-slate-500">
                                                                        Area:
                                                                        <span class="font-semibold">
                                                                            {{ $camp->emArea_display ?: $camp->emArea ?: 'Not specified' }}
                                                                        </span>
                                                                    </div>
                                                                </div>

                                                                <div>
                                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Requested</div>
                                                                    <div class="mt-1 text-sm font-semibold text-slate-700">
                                                                        {{ $dateFmt($camp->emRequest ?? $camp->created_at) }}
                                                                    </div>
                                                                </div>

                                                                <div>
                                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Sent</div>
                                                                    <div class="mt-1 text-sm font-semibold text-slate-700">
                                                                        {{ $dateFmt($camp->emComplete) }}
                                                                    </div>
                                                                </div>

                                                                <div>
                                                                    <div class="text-xs font-bold uppercase tracking-wide text-slate-400">Emails</div>
                                                                    <div class="mt-1 text-sm font-black text-slate-900">
                                                                        {{ number_format((int) ($camp->totalEmails ?? 0)) }}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        @endforeach
                                            ...
                                            @else
                                                <div class="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                                                    This flyer has not had an email campaign requested yet.
                                                </div>
                                            @endif
                                        </div>
                                    </div>
